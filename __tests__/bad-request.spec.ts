...
describe('HttpError.BadRequest', () => {
    it('should handle explicit HttpError.NotFound', async () => {
        ...
        .get('/foo', () => {
            throw new HttpError.BadRequest('This is a bad request', [{field: 'name', message: 'Name is required'}]);
        })
        ...
        expect(json).toEqual(
            new HttpError.BadRequest('This is a bad request', [{field: 'name', message: 'Name is required'}]).toJSON(
        ))
    });
